	...
	public String search() {
		...
		if(SgfUtil.isAdministrador(user) ||  SgfUtil.isCoordenador(user)){
			...
			if(ua != null){
				...
				//this.entities = service.findByUGStatus(ua.getUg().getId(), "true");
			}
		}
	...
	public String searchUsuarios(){
		...
		if(role.getAuthority().equals("ROLE_ADMIN") || role.getAuthority().equals("ROLE_COORD_TRANSP")){
			...
			if(this.filter == ""){
				...
				this.entities = service.findByUGNameStatus(null, null, str);

			} else {

				this.entities = service.findByUGNameStatus(null, this.filter, str);
			}
			...
			if(ua != null){
				...
				if(this.filter == ""){
					...
					this.entities = service.findByUGNameStatus( ua.getUg().getId(), null, str);

				} else {

					this.entities = service.findByUGNameStatus( ua.getUg().getId(), this.filter, str);
				}
			}
		}
	...
	public String searchByCpf(){
		...
		try{
			...
			if(pessoa != null){
				...
				this.usuarioCadastrado = service.findPessoaByCpf(pessoa.getCpf());
